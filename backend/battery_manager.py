...
class BatteryManager:
    def __init__(self):
        self.state = {
            ...
            "selected_cell": None,  # {brand, model} picked from cell DB
            "cell_specs": {},       # parsed specs from CSV: capacity_mah, nominal_v, …
            "total_cells": None,
            "num_groups": None,
    ...
    def set_selected_cell(self, brand: str, model: str, specs: dict):
        self.state["selected_cell"] = {"brand": brand, "model": model}
        self.state["cell_specs"] = specs

    def clear_selected_cell(self):
        self.state["selected_cell"] = None
        self.state["cell_specs"] = {}
    ...
    def generate_header(self) -> str:
        ...
        selected   = s.get("selected_cell")
        cell_specs = s.get("cell_specs") or {}
        ...
        if selected and cell_specs:
            lines += [
                "",
                f'// Cell model       : {selected["brand"]} {selected["model"]}',
            ]
            if "capacity_mah" in cell_specs:
                lines.append(f'#define CELL_CAPACITY_MAH    {int(cell_specs["capacity_mah"])}')
            if "nominal_v" in cell_specs:
                lines.append(f'#define CELL_NOMINAL_V       {cell_specs["nominal_v"]:.2f}f')
            if "max_charge_v" in cell_specs:
                lines.append(f'#define CELL_MAX_CHARGE_V    {cell_specs["max_charge_v"]:.2f}f')
            if "cutoff_v" in cell_specs:
                lines.append(f'#define CELL_CUTOFF_V        {cell_specs["cutoff_v"]:.2f}f')
            if "max_charge_ma" in cell_specs:
                lines.append(f'#define CELL_MAX_CHARGE_A    {cell_specs["max_charge_ma"] / 1000:.3f}f')
            if "max_discharge_ma" in cell_specs:
                lines.append(f'#define CELL_MAX_DISCHARGE_A {cell_specs["max_discharge_ma"] / 1000:.3f}f')
